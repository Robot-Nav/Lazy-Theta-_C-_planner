...
void ThetaStar::setCostmap(GridMap * costmap)
{
  costmap_ = costmap;
  size_x_ = static_cast<int>(costmap->getSizeInCellsX());
  size_y_ = static_cast<int>(costmap->getSizeInCellsY());
}

void ThetaStar::setStartAndGoal(int start_x, int start_y, int goal_x, int goal_y)
{
  src_ = {start_x, start_y};
  dst_ = {goal_x, goal_y};
}

bool ThetaStar::setStartAndGoalWorld(double start_wx, double start_wy,
                                     double goal_wx, double goal_wy)
{
  unsigned int sx, sy, gx, gy;
  if (!costmap_->worldToMap(start_wx, start_wy, sx, sy) ||
    !costmap_->worldToMap(goal_wx, goal_wy, gx, gy))
  {
    return false;
  }
  src_ = {static_cast<int>(sx), static_cast<int>(sy)};
  dst_ = {static_cast<int>(gx), static_cast<int>(gy)};
  return true;
}

void ThetaStar::setConfig(const PlannerConfig & config)
{
  w_traversal_cost_ = config.w_traversal_cost;
  w_euc_cost_ = config.w_euc_cost;
  how_many_corners_ = (config.how_many_corners == 4 || config.how_many_corners == 8) ?
    config.how_many_corners : 8;
  allow_unknown_ = config.allow_unknown;
  terminal_checking_interval_ = config.terminal_checking_interval;
  w_heuristic_cost_ = (w_euc_cost_ < 1.0) ? w_euc_cost_ : 1.0;
}

bool ThetaStar::generatePath(std::vector<coordsW> & raw_path, std::function<bool()> cancel_checker)
{
  if (src_.x == dst_.x && src_.y == dst_.y) {
    raw_path.clear();
    coordsW world{};
    costmap_->mapToWorld(src_.x, src_.y, world.x, world.y);
    raw_path.push_back(world);
    nodes_opened = 0;
    return true;
  }
...
void ThetaStar::resetContainers()
{
  ...
  // Resize node_position_ to match current costmap size
  int total_size = curr_size_x * curr_size_y;
  node_position_.resize(total_size, nullptr);
  ...
  nodes_data_.clear();
  nodes_data_.reserve(total_size);
...
{
  // This function is now handled by resetContainers
}
...
void ThetaStar::clearStart()
{
  ...
  costmap_->setCost(mx_start, my_start, FREE_SPACE);
}
